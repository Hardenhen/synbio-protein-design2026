#!/usr/bin/env python3
"""Team2 local round2 micro-hillclimb (~135 candidates)

Mature strategy learned from SnowFold:
- Use previous Top3 as parents
- Focus on low/mid-low temps [0.05, 0.1, 0.2]
- Keep M + chromophore key fixed positions
"""
...
WORK = "D:/workspace/team2_local_r2"
MPNN_DIR = "C:/proteinmpnn_r10"
os.makedirs(WORK, exist_ok=True)
for d in ["pdbs","mpnn_out","results"]:
    os.makedirs(os.path.join(WORK,d), exist_ok=True)

PARENTS = json.load(open(r"D:/workspace/team2_local/final_6_local.json"))[:3]
TEMPS = [0.05, 0.1, 0.2]
NUM_SEQ_PER_TEMP = 15
...
def run_mpnn(pdb, name):
    ...
    cmd=[sys.executable, os.path.join(MPNN_DIR,"protein_mpnn_run.py"),
         ...
         "--sampling_temp", " ".join(map(str,TEMPS)), "--seed", "2803", "--suppress_print", "1"]
...
if __name__ == "__main__":
    t0=time.time()
    print(f"Team2 Local R2 Start: {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    ...
    json.dump(all_passed, open(os.path.join(WORK,"all_passed_local_r2.json"),"w"), indent=2)
    final6=all_passed[:6]
    json.dump(final6, open(os.path.join(WORK,"final_6_local_r2.json"),"w"), indent=2)
    with open(os.path.join(WORK,"submission_local_r2.csv"),"w",newline="") as f:
